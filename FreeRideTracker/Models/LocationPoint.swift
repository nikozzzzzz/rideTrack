...
final class LocationPoint {
    ...
    private static func validateLatitude(_ lat: Double) -> Double {
        guard lat.isFinite else {
            AppLogger.warning("Invalid latitude (not finite), using 0.0", category: AppLogger.data)
            return 0.0
        }
        if !AppConstants.Location.validLatitudeRange.contains(lat) {
            AppLogger.warning("Latitude \(lat) out of range, clamping to valid range", category: AppLogger.data)
    ...
    private static func validateLongitude(_ lon: Double) -> Double {
        guard lon.isFinite else {
            AppLogger.warning("Invalid longitude (not finite), using 0.0", category: AppLogger.data)
            return 0.0
        }
        if !AppConstants.Location.validLongitudeRange.contains(lon) {
            AppLogger.warning("Longitude \(lon) out of range, clamping to valid range", category: AppLogger.data)
    ...
    private static func validateAltitude(_ alt: Double) -> Double {
        guard alt.isFinite else {
            AppLogger.warning("Invalid altitude (not finite), using 0.0", category: AppLogger.data)
            return 0.0
        }
        if alt < AppConstants.Location.minReasonableAltitude || alt > AppConstants.Location.maxReasonableAltitude {
            AppLogger.warning("Altitude \(alt) out of reasonable range, clamping", category: AppLogger.data)
    ...
    private static func validateSpeed(_ speed: Double) -> Double {
        guard speed.isFinite else {
            AppLogger.warning("Invalid speed (not finite), using 0.0", category: AppLogger.data)
            return 0.0
        }
        if speed < 0 || speed > AppConstants.Location.maxReasonableSpeed {
            AppLogger.warning("Speed \(speed) out of reasonable range, clamping", category: AppLogger.data)
